(() => {
  ...
  if (page === "login") {
    $("#btnLogin")?.addEventListener("click", async () => {
      ...
      try {
        ...
        const out = await apiPost("dealer_login_password", {
          dealer_email,
          dealer_phone,
          password
        });
        ...
        setSession({
          token: out.token,
          dealer_id: out.dealer_id || "",
          dealer_email: out.dealer_email,
          dealer_phone: out.dealer_phone,
  ...
  if (page === "register") {
    $("#btnRegister")?.addEventListener("click", async () => {
      ...
      try {
        ...
        const out = await apiPost("dealer_register_password", {
          dealer_name,
          dealer_email,
          dealer_phone,
          password: pass
  ...
  if (page === "portal") {
    ...
    const who = $("#who");
    if (who) who.textContent = (s.dealer_name || "") + " — " + s.dealer_email;
  ...
  if (page === "new") {
    ...
    $("#btnNext").addEventListener("click", () => {
      ...
      if (stepN === 1) {
        for (const k of req1) {
          if (!safe(fd.get(k))) return setMsg(m, "أكمل بيانات الفاتورة.", false);
        }
      }

      if (stepN === 2) {
        for (const k of req2) {
          if (!safe(fd.get(k))) return setMsg(m, "أكمل بيانات العميل.", false);
        }

        if (!isEmail(fd.get("customer_email"))) {
          return setMsg(m, "البريد الإلكتروني غير صحيح.", false);
        }
        ...
        $("#newReview").innerHTML =
          ...
          `رقم اللوحة: ${v("plate_number")}<br>` +
          `نوع السيارة: ${v("car_type")}<br>` +
          `الرول: ${v("roll_number")}<br>` +
          `فئة المنتج: ${v("product_category")}<br>` +
    ...
    form.addEventListener("submit", async (e) => {
      ...
      for (const k of req) {
        if (!safe(fd.get(k))) return setMsg(m, "يرجى تعبئة جميع الحقول المطلوبة.", false);
      }
      ...
      const payload = {
        ...
        plate_number: safe(fd.get("plate_number")),
        car_type: safe(fd.get("car_type")),
        roll_number: safe(fd.get("roll_number")),
        product_category: safe(fd.get("product_category")),
  ...
  if (page === "edit") {
    ...
    $("#btnBackHome2")?.addEventListener("click", () => {
      window.location.href = "portal.html";
    });
    ...
    $("#btnLoadEdit")?.addEventListener("click", async () => {
      ...
      try {
        const out = await apiPost("submission_get_for_edit", {
          token: s.token,
          invoice_number: inv
        });
        fillEdit(out.row);
    ...
    function fillEdit(r) {
      ...
      form.plate_number.value = r.plate_number || "";
      form.car_type.value = r.car_type || "";
      form.roll_number.value = r.roll_number || "";
      form.product_category.value = r.product_category || "";
